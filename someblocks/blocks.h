...
static const Block blocks[] = {
	...
	{ " 󰎆  ", "playerctl metadata -f '{{trunc(artist,20)}} 「{{trunc(title,20)}}」'", 10,    0  },
	{ "   ", "free -h | awk '/^Mem/ { print $3\"/\"$2 } '",                          60,    0  },
  { "   ", "cut -d' ' -f1 /proc/loadavg",                                          60,    0  },
  { "   ", "sensors | awk '/^Core 0/ {print $3 }' | cut -c2-",                     60,    0  },
